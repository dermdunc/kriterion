...
## Status: V0 experiment complete, headline result in; V1 instrument increments landed
...
research preserved intact at `docs/lab.html`.

V1.1 then made that journey a **projection** rather than a parallel narrative: one read-only
`DecisionState` per run (ADR-011), narrative integrity as a deterministic invariant with
publication failing closed (ADR-012), and the page generated by `kriterion decision-page` with a
test asserting the committed HTML is byte-identical to a fresh render. V1.1.1 anchored the two
principles behind that work as standing architecture rather than page-scoped decisions (ADR-013),
and stated plainly where they are mechanically enforced and where they are only held by review:
see [`docs/architecture.md`](docs/architecture.md).

Two things are deliberately still missing, both named in
[`docs/next-actions.md`](docs/next-actions.md): no accountable human has recorded a decision on the
canonical case, and the narrative-integrity checker covers the public decision page but not the
per-run report, the Lab page or CLI output. The V0 experiment record below is unchanged.
